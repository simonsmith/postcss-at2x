...
function at2x({identifier = '@2x', detectImageSize = false} = {}) {
  return function(root) {
    ...
    root.walkRules((rule) => {
      ...
      rule.walkDecls(/^background/, (decl) => {
        ...
        const promise = getBackgroundImageSize(decl, backgroundSize, detectImageSize)
          .then((size) => {
            if (size) {
...
function getBackgroundImageSize(decl, existingBackgroundSize, detectImageSize) {
  if (!detectImageSize) {
