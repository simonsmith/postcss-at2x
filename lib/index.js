    ...
    Once(root, {result, postcss: api}) {
      ...
      root.walkRules((rule) => {
        ...
        rule.walkDecls(/^background/, (decl) => {
          ...
          if (
            skipMissingRetina &&
            !retinaImageExists(retinaImages, decl.source, resolveImagePath)
          ) {
            return;
          }
...
function addRetinaRule(ruleContainer, mediaParent, decl, retinaImages, size, api) {
  // Construct a duplicate rule but with the image urls
  // replaced with retina versions
  const retinaRule = api.rule({selector: decl.parent.selector});

  retinaRule.append(api.decl({
    prop: 'background-image',
    value: retinaImages,
  }));

  if (size) {
    retinaRule.append(api.decl(size));
  }
  ...
  const mediaAtRule = api.atRule({name: 'media', params});
